...
function MBTIPredictor() {
  ...
  const handleSubmit = async () => {
    const response = await fetch('http://predictions76.railway.app/api/predict/', {
      method: 'POST',
  ...
  const handleCheckQualities = async () => {
    const response = await fetch(`http://predictions76.railway.app/api/qualities/?type=${mbti}`);
    const data = await response.json();
